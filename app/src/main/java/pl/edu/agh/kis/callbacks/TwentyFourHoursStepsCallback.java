...
public class TwentyFourHoursStepsCallback implements Callback {

    private final String TAG = getClass().getSimpleName();

    @Override
    public void execute(Attribute attribute, WorkingMemory workingMemory) {
        Log.i(TAG, "Executing TwentyFourHoursStepsCallback for " + attribute.getName());
        try {
            workingMemory.setAttributeValue(attribute, new SimpleNumeric(resolveTwentyFourHoursSteps()), false);
        } catch (AttributeNotRegisteredException | NotInTheDomainException e) {
            Log.i(TAG, "Error occurred: ", e);
        }
    }

    private double resolveTwentyFourHoursSteps() {
        List<Map.Entry<LocalDateTime, Float>> todays = StatisticKeeper.getStepsData().entrySet().stream()
                .filter(entry -> entry.getKey().isAfter(LocalDateTime.now().minusDays(1)))

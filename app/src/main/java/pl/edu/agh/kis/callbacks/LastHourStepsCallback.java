...
public class LastHourStepsCallback implements Callback {

    private final String TAG = getClass().getSimpleName();

    @Override
    public void execute(Attribute attribute, WorkingMemory workingMemory) {
        Log.i(TAG, "Executing LastHourStepsCallback for " + attribute.getName());
        try {
            workingMemory.setAttributeValue(attribute, new SimpleNumeric(resolveLastHourSteps()), false);
        } catch (AttributeNotRegisteredException | NotInTheDomainException e) {
            Log.i(TAG, "Error occurred: ", e);
        }
    }

    private double resolveLastHourSteps() {
        List<Map.Entry<LocalDateTime, Float>> todays = StatisticKeeper.getStepsData().entrySet().stream()
                .filter(entry -> entry.getKey().isAfter(LocalDateTime.now().minusHours(1)))
